    ...
    <div class="form-card">
        <form action="{{ route('admin.organisasi.update', $organisasi) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group" style="background: #f8fafc; padding: 1rem; border-radius: 8px; border: 1.5px dashed #cbd5e1;">
                ...
                <div class="form-helper">Pilih anggota dari database web untuk auto-fill data. *(Anggota lain yang sudah terdaftar di struktur disembunyikan)*</div>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="kabupaten" name="kabupaten" class="form-select select2 @error('kabupaten') error @enderror">
                    ...
                    @foreach(\App\Helpers\WilayahHelper::getDaftarKabupatenKota() as $kabKey => $kabVal)
                        <option value="{{ $kabKey }}" {{ old('kabupaten', $organisasi->kabupaten) == $kabKey ? 'selected' : '' }}>{{ $kabVal }}</option>
                    @endforeach
                </select>
                @error('kabupaten')
                    <div class="error-message">{{ $message }}</div>
