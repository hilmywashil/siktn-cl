/**
 * @fileoverview HTML5 QRCode generator library
 * @author davidshimjs
 * @see <a href="http://www.davidshimjs.com/qrcode">http://www.davidshimjs.com/qrcode</a>
 */
var QRCode;
(function () {
	function QRCodeAlg(typeNumber, errorCorrectLevel) {
	...
	QRCodeAlg.prototype = {
		addData: function (data) {
			var newData = new QRCodeData(data);
		...
		makeImpl: function (test, maskPattern) {
			...
			if (this.dataCache == null) {
				this.dataCache = QRCodeAlg.createData(this.typeNumber, this.errorCorrectLevel, this.dataList);
			}
    ...
    QRCodeAlg.createData = function (typeNumber, errorCorrectLevel, dataList) {
        ...
        return QRCodeAlg.createBytes(buffer, rsBlocks);
    };

    QRCodeAlg.createBytes = function (buffer, rsBlocks) {
        var offset = 0;
        var maxDcCount = 0;
        var maxEcCount = 0;
    ...
    QRCode = function (element, option) {
        ...
        if (option) {
            for (var i in option) _htOption[i] = option[i];
        }
        ...
        try {
            var text = _htOption.text || "https://karangtaruna.or.id";
            var _oQRCode = null;
            for (var t = 1; t <= 40; t++) {
                try {
                    var testQr = new QRCodeAlg(t, _htOption.correctLevel);
                    testQr.addData(text);
                    testQr.make();
                    _oQRCode = testQr;
                    break;
                } catch (err) {
                    // Try next higher QR type number
                }
            }
            if (!_oQRCode) return;
            ...
            canvas.width = _htOption.width || 120;
            canvas.height = _htOption.height || 120;
            var ctx = canvas.getContext("2d");

            var tileW = canvas.width / _oQRCode.getModuleCount();
            var tileH = canvas.height / _oQRCode.getModuleCount();

            for (var row = 0; row < _oQRCode.getModuleCount(); row++) {
                for (var col = 0; col < _oQRCode.getModuleCount(); col++) {
                    ctx.fillStyle = _oQRCode.isDark(row, col) ? _htOption.colorDark : _htOption.colorLight;
